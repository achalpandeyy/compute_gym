...
void run()
{
  ...
  auto initBWps = ((mibibytes ? std::pow(2.0, -30.0) : 1.0E-6) * (3 * sizeof(T) * ARRAY_SIZE)) / initElapsedS;
  auto readBWps = ((mibibytes ? std::pow(2.0, -30.0) : 1.0E-6) * (3 * sizeof(T) * ARRAY_SIZE)) / readElapsedS;
  ...
  else
  {
    std::cout << "Init: "
      ...
      << " s (="
      << initBWps
      << (mibibytes ? " GBytes/sec" : " MBytes/sec")
      << ")" << std::endl;
    std::cout << "Read: "
      << std::setw(7)
      << readElapsedS
  ...
  else
  {
    std::cout
      ...
      << std::left << std::setw(12) << ((mibibytes) ? "GBytes/sec" : "MBytes/sec")
      ...
      else
      {
        std::cout
          ...
          << std::left << std::setw(12) << std::setprecision(3) << 
            ((mibibytes) ? std::pow(2.0, -30.0) : 1.0E-6) * sizes[i] / (*minmax.first)
